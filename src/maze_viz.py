...
class Visualizer(object):
    ...
    def show_maze_solution(self):
        ...
        self.ax.add_patch(plt.Circle(((self.maze.solution_path[0][1] + 0.5)*self.cell_size,
                                      (self.maze.solution_path[0][0] + 0.5)*self.cell_size), 0.2*self.cell_size,
                                     fc=(0, circle_num/(len(self.maze.solution_path)),
                                         0), alpha=0.4))

        for i in range(1, self.maze.solution_path.__len__()):
                circle_num += 1
                self.ax.add_patch(plt.Circle(((self.maze.solution_path[i][1] + 0.5)*self.cell_size,
                    (self.maze.solution_path[i][0] + 0.5)*self.cell_size), 0.2*self.cell_size,
                    fc = (0, circle_num/(len(self.maze.solution_path)), 0), alpha = 0.4))

        self.ax.set_title("Cost: {}".format(self.maze.cost), fontname="serif", fontsize=19)
        # Display the plot to the user
        plt.show()
    ...
    def animate_maze_solution(self):
        ...
        indicator = plt.Rectangle((self.maze.solution_path[0][0]*self.cell_size,
                                   self.maze.solution_path[0][1]*self.cell_size), self.cell_size, self.cell_size,
                                  fc="purple", alpha=0.6)
        self.ax.add_patch(indicator)
        ...
        def animate_squares(frame):
            ...
            if frame > 0:
                ...
                self.squares["{},{}".format(self.maze.solution_path[frame][0],
                                       self.maze.solution_path[frame][1])].set_visible(False)
            return []

        def animate_indicator(frame):
            """Function to animate where the current search is happening."""
            indicator.set_xy((self.maze.solution_path[frame][1] * self.cell_size,
                              self.maze.solution_path[frame][0] * self.cell_size))
            return []
