...
from src.solver import q_learning
from src.solver import q_learning_path
...
class MazeManager(object):
    ...
    def solve_maze(self, maze_id, method, neighbor_method="fancy"):
        ...
        if method == "Q_learning":
            while True:
                try:
                    episodes_num = int(input("Enter the number of episodes: "))
                    if episodes_num <= 0:
                        raise ValueError
                    break
                except ValueError:
                    print("Invalid input. Please enter a valid integer that bigger than 0.")
            ...
            q_table = q_learning(maze, episodes_num, learning_rate, discount_factor, exploration_rate)
            found_path, found_cost = q_learning_path(maze, q_table)
            maze.solution_path = found_path
            maze.cost = found_cost
